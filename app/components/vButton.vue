...
<template>
  <div class="relative inline-block" @mouseenter="hovered = true" @mouseleave="hovered = false">
    <button
      v-if="props.showButton"
      :disabled="props.disabled"
      v-bind="$attrs"
      :class="[
        'relative z-10 cursor-pointer font-bold px-4 py-2 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]',
        !($attrs.class as string)?.includes('bg-')
          ? 'text-white dark:text-black bg-black dark:bg-white'
          : '',
        props.disabled ? 'opacity-50 cursor-not-allowed' : 'hover:scale-x-105 hover:scale-y-[0.95]',
        !($attrs.class as string)?.includes('rounded-') ? 'rounded-xl' : '',
        $attrs.class,
      ]"
    >
      <slot />
