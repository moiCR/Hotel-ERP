...
const confirmDelete = async () => {
    ...
    loading.value = true;
    try {
        const response = await fetch("/api/sede/delete", {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id: props.sede.id }),
        });
        ...
        if (data.success) {
            ...
            useToast().showToast(data.message || "Error al eliminar la sede.", "error");
        }
    } catch (e) {
        const { useToast } = await import('#imports');
...
<template>
    <VModal
        :is-open="isOpen"
        :origin-el="originEl"
        :layout-id="sede ? `delete-sede-${sede.id}` : 'delete-sede'"
        @close="emit('close')"
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-sm text-gray-400">
                        Esta acción no se puede deshacer. El hotel sede en
