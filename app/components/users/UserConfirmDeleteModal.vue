...
const confirmDelete = async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            useToast().showToast(data.message || "Error al eliminar el usuario", "error");
        }
    } catch (e) {
        const { useToast } = await import('#imports');
...
<template>
    <VModal
        :is-open="isOpen"
        :origin-el="originEl"
        :layout-id="user ? `delete-user-${user.id}` : 'delete-user'"
        @close="emit('close')"
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-sm text-gray-400">
                        ...
                        >
                        perderá todo acceso al sistema de forma permanente.
                    </p>
                </div>
